...
(function($){
  ...
  function updateDataToSent(input){
  	container = $(input).parents('ul.autobox-hldr')
  	exp = /^[^\<]+/;
  	tags = '';
  	container.find('li.bit-box').each(function(){
  		if(tag_text = $(this).html().match(exp)[0])
  		{ 
	  		if(tags == '')
	  			tags = tag_text;
	  		else
  ...
  $.fn.autoboxMode=function(container, input, size, opt){
    ...
    input
      .bind("keydown.autobox", function(e){
        ...
        if(k == KEY.ESC){ $("body").trigger("cancel.autobox"); updateDataToSent() }
